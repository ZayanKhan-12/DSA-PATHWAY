...
export default function SurroundedRegionsCardWiring() {
  useEffect(() => {
    const internalRoute = "/practice/trees-recursion/surrounded-regions";
    ...
    const findCard = () => {
      ...
      ).filter((node) => normalize(node.textContent || "") === "Surrounded Regions");
      ...
      for (const titleNode of titleNodes) {
        ...
        for (let depth = 0; depth < 6 && current; depth++) {
          ...
          const isExactCard =
            text.includes("Surrounded Regions") &&
            text.includes("DFS boundary-connected cells before flipping") &&
            !text.includes("Clone Graph") &&
    ...
    const wireCard = () => {
      ...
      if (target.dataset.surroundedRegionsCardWired === "true") return true;

      target.dataset.surroundedRegionsCardWired = "true";
      target.style.cursor = "pointer";
      target.setAttribute("role", "link");
      target.setAttribute("tabindex", "0");
      target.setAttribute("aria-label", "Open Surrounded Regions practice page");
