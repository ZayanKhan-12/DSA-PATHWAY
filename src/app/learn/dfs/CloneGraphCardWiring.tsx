...
export default function CloneGraphCardWiring() {
  useEffect(() => {
    const internalRoute = "/practice/trees-recursion/clone-graph";
    ...
    const findCard = () => {
      ...
      ).filter((node) => normalize(node.textContent || "") === "Clone Graph");
      ...
      for (const titleNode of titleNodes) {
        ...
        for (let depth = 0; depth < 6 && current; depth++) {
          ...
          const isExactCard =
            ...
            text.includes("DFS over graph structure while building copies") &&
    ...
    const wireCard = () => {
      ...
      if (target.dataset.cloneGraphCardWired === "true") return true;

      target.dataset.cloneGraphCardWired = "true";
      target.style.cursor = "pointer";
      target.setAttribute("role", "link");
      target.setAttribute("tabindex", "0");
      target.setAttribute("aria-label", "Open Clone Graph practice page");
