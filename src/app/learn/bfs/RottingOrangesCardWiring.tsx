...
export default function RottingOrangesCardWiring() {
  useEffect(() => {
    const internalRoute = "/practice/bfs/rotting-oranges";
    ...
    const findCard = () => {
      ...
      ).filter((node) => normalize(node.textContent || "") === "Rotting Oranges");
      ...
      for (const titleNode of titleNodes) {
        ...
        for (let depth = 0; depth < 6 && current; depth++) {
          ...
          const isExactCard =
            ...
            text.includes("Multi-source BFS") &&
            text.includes("rotten cell") &&
    ...
    const wireCard = () => {
      ...
      if (target.dataset.rottingOrangesCardWired === "true") return true;

      target.dataset.rottingOrangesCardWired = "true";
      target.style.cursor = "pointer";
      target.setAttribute("role", "link");
      target.setAttribute("tabindex", "0");
      target.setAttribute("aria-label", "Open Rotting Oranges practice page");
