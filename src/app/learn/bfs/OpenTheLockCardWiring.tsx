...
export default function OpenTheLockCardWiring() {
  useEffect(() => {
    const internalRoute = "/practice/bfs/open-the-lock";
    ...
    const isCorrectCardText = (text: string) => {
      ...
      return (
        clean.includes("open the lock") &&
        clean.includes("bfs over states") &&
        clean.includes("deadends") &&
    ...
    const findCard = () => {
      ...
      ).filter((node) => normalize(node.textContent || "") === "open the lock");
    ...
    const wireCard = () => {
      ...
      target.dataset.openTheLockCardWired = "true";
      target.style.cursor = "pointer";
      target.setAttribute("role", "link");
      target.setAttribute("tabindex", "0");
      target.setAttribute("aria-label", "Open Open the Lock practice page");
