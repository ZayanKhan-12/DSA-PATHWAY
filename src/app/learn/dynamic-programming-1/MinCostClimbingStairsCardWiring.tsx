...
export default function MinCostClimbingStairsCardWiring() {
  useEffect(() => {
    const internalRoute = "/practice/dynamic-programming-1/min-cost-climbing-stairs";
    ...
    const isCorrectCardText = (text: string) => {
      ...
      return (
        ...
        clean.includes("state transition") &&
        clean.includes("one-dimensional positions") &&
    ...
    const findCard = () => {
      ...
      ).filter((node) => normalize(node.textContent || "") === "min cost climbing stairs");
    ...
    const wireCard = () => {
      ...
      target.dataset.minCostClimbingStairsCardWired = "true";
      target.style.cursor = "pointer";
      target.setAttribute("role", "link");
      target.setAttribute("tabindex", "0");
      target.setAttribute("aria-label", "Open Min Cost Climbing Stairs practice page");
