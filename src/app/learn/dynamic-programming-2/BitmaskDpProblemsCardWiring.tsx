...
export default function BitmaskDpProblemsCardWiring() {
  useEffect(() => {
    const internalRoute = "/practice/dynamic-programming-2/bitmask-dp-problems";
    ...
    const isCorrectCardText = (text: string) => {
      ...
      return (
        clean.includes("bitmask dp problems") &&
        clean.includes("subset-state optimization") &&
        clean.includes("higher complexity") &&
    ...
    const findCard = () => {
      ...
      ).filter((node) => normalize(node.textContent || "") === "bitmask dp problems");
    ...
    const wireCard = () => {
      ...
      target.dataset.bitmaskDpProblemsCardWired = "true";
      target.style.cursor = "pointer";
      target.setAttribute("role", "link");
      target.setAttribute("tabindex", "0");
      target.setAttribute("aria-label", "Open Bitmask DP Problems practice page");
