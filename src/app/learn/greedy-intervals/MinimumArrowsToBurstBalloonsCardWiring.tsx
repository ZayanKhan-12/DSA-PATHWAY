...
export default function MinimumArrowsToBurstBalloonsCardWiring() {
  useEffect(() => {
    const internalRoute = "/practice/greedy-intervals/minimum-arrows-to-burst-balloons";
    ...
    const isCorrectCardText = (text: string) => {
      ...
      return (
        clean.includes("minimum arrows to burst balloons") &&
        !clean.includes("maximum number of non-overlapping intervals") &&
    ...
    const findCard = () => {
      ...
      ).filter((node) => normalize(node.textContent || "") === "minimum arrows to burst balloons");
    ...
    const wireCard = () => {
      ...
      target.dataset.minimumArrowsToBurstBalloonsCardWired = "true";
      target.style.cursor = "pointer";
      target.setAttribute("role", "link");
      target.setAttribute("tabindex", "0");
      target.setAttribute("aria-label", "Open Minimum Arrows to Burst Balloons practice page");
