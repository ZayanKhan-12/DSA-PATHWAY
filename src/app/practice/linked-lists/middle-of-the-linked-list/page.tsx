...
public:
  ...
  rust: `impl Solution {
    pub fn middle_node(head: Option<Box<ListNode>>) -> Option<Box<ListNode>> {
        ...
        for value in values[middle..].iter().rev() {
...
const TEST_CASES: TestCase[] = [
  { id: 1, head: [1, 2, 3, 4, 5], expected: [3, 4, 5] },
  { id: 2, head: [1, 2, 3, 4, 5, 6], expected: [4, 5, 6] },
  { id: 3, head: [1], expected: [1] },
  { id: 4, head: [1, 2], expected: [2] },
];
...
function executeJavaScript(code: string, test: TestCase) {
  try {
    const middleNode = new Function(`
      "use strict";
      ${code}
      return middleNode;
    `)() as (head: ListNode | null) => ListNode | null;

    if (typeof middleNode !== "function") {
      return {
        pass: false,
        actual: [] as number[],
        error: "middleNode function was not found. Keep the function named middleNode.",
      };
    }

    const head = buildLinkedList(test.head);
    const middleHead = middleNode(head);
    const actual = linkedListToArray(middleHead);
...
export default function MiddleOfTheLinkedListPracticePage() {
          ...
          <section className="flex min-h-0 flex-col overflow-hidden border border-border bg-black/20">
            <div className="border-b border-border bg-black/20 px-5 py-4">
              ...
              <p className="max-w-3xl text-sm leading-7 text-muted-foreground md:text-base">
                Find the middle node of a linked list using the fast/slow midpoint pattern.
              </p>
            </div>
              ...
              <div className="space-y-5 text-sm leading-7 text-muted-foreground">
                <div className="terminal-frame p-4">
                  ...
                  <p className="mt-2">
                    ...
                    <span className="text-foreground">second middle</span> node.
                  </p>
                  <p className="mt-2">
                    Move <span className="text-foreground">slow</span> one step and{" "}
                    <span className="text-foreground">fast</span> two steps. When fast reaches the end, slow is at the middle.
                  </p>
                </div>
...
Output: [3,4,5]`}
                ...
                <div className="terminal-frame p-4">
                  ...
                  <ul className="list-disc space-y-2 pl-5">
                    <li>The number of nodes is between 1 and 100</li>
                    <li>1 ≤ Node.val ≤ 100</li>
                    <li>If the length is even, return the second middle node</li>
            ...
            <div className="grid h-[210px] flex-none xl:grid-cols-2">
              ...
              <div className="px-4 py-4">
                ...
                <div className="terminal-frame h-[150px] overflow-auto p-3 text-xs leading-6 text-muted-foreground">
                  <p>{resultText}</p>
                  <p className="mt-3">Expected approach: fast/slow midpoint</p>
                  <p>Time: O(n)</p>
                  <p>Space: O(1)</p>
