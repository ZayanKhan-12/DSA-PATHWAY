...
const Section = ({ id, idx, title, children }: { id: string; idx: string; title: string; children: React.ReactNode }) => (
          ...
          <div className="mt-8 grid md:grid-cols-[260px_1fr] gap-8">
            ...
            <div>
              <Section id="root-problem" idx="01" title="The Root Problem">
                <p>
                  You're given a maze. Or a social network. Or a chessboard. The question is always some variant of:
                </p>
...
queue  →  BFS  →  goes wide first`}</Code>
              ...
              <Section id="walkthrough" idx="04" title="Step-by-Step Example">
                <p>
                  The interactive visualizer is the best way to feel this. Press STEP and watch the queue mutate.
                </p>
                <Link
                  to="/visualize/bfs"
...
visit order: A B C D E F G H I`}</Code>
              ...
              <Section id="code" idx="05" title="Code Implementation">
                <p>The canonical TypeScript implementation. Note: <span className="text-primary">mark visited when you enqueue, not when you dequeue.</span> Otherwise you'll add the same node twice.</p>
                <Code>{`function bfs(
  start: number,
  ...
  while (queue.length > 0) {
    const node = queue.shift()!;   // O(n) — see note below
    order.push(node);
  ...
  return order;
                ...
                <p>
                  ...
                  In production, use a real deque (head pointer + array, or a linked list) to keep BFS at O(V+E).
                </p>
              </Section>
...
E = number of edges`}</Code>
                <p>
                  Why? Each node enters the queue exactly once (V work). Each edge is examined twice — once from each endpoint (2E work).
                  The visited set holds at most V entries. That's it.
                </p>
              </Section>
